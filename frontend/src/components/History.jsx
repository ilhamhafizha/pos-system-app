...
const OrderHistory = () => {
  ...
  const handleDetailClick = async (order_number) => {
    try {
      const token = JSON.parse(localStorage.getItem("user"))?.token;
      const res = await axios.get(
        `http://localhost:3000/cashier/sales-report/${order_number}/receipt`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      navigate("/cashier/receipt", { state: { receipt: res.data.data } });
    } catch (err) {
              ...
              sales.map((tx, idx) => (
                <tr key={idx} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3 text-center">
                    <FiExternalLink
                      onClick={() => handleDetailClick(tx.order_number)}
